...
const STREAM_IDLE_TIMEOUT_MS = Number.parseInt(
	process.env.TEAMFLOW_STREAM_IDLE_TIMEOUT_MS ?? "300000",
	10,
);
const STREAM_IDLE_TICK_MS = Number.parseInt(
	process.env.TEAMFLOW_STREAM_IDLE_TICK_MS ?? "15000",
	10,
);
		...
		setInterval(() => {
			...
			process.stderr.write(
				`[agent-watchdog] stream idle ${idleMs}ms > ${STREAM_IDLE_TIMEOUT_MS}ms ` +
					`(set TEAMFLOW_STREAM_IDLE_TIMEOUT_MS=0 to disable); aborting stalled provider request\n`,
			);
