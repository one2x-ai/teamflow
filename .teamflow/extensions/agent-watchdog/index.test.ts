 ...
 *
 * The module reads TEAMFLOW_STREAM_IDLE_* at load, so env is set before the
...
test("aborts a stalled provider request at TTFT (no message_update)", async () => {
	process.env.TEAMFLOW_STREAM_IDLE_TIMEOUT_MS = "120";
	process.env.TEAMFLOW_STREAM_IDLE_TICK_MS = "30";
