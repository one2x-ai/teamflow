#!/usr/bin/env python3
"""Mechanical state layer for teamflow handoffs.
...
Layout below ``.teamflow/runs/code/``::
...
BUDGET_REASONS = ("CONTEXT_BUDGET_EXCEEDED", "RECALL_BUDGET_EXCEEDED")
...
DEFAULT_RUNS_DIR = ".teamflow/runs/code"
...
def _stale_seconds():
    raw = os.environ.get("TEAMFLOW_HANDOFF_TIMEOUT_SECONDS") or os.environ.get(
...
def cmd_handoff_status(args, paths):
    handoff_id = args.id or os.environ.get("TEAMFLOW_HANDOFF_ID")
    if handoff_id:
...
def _resolve_handoff_id(args):
    handoff_id = args.id or os.environ.get("TEAMFLOW_HANDOFF_ID")
    if not handoff_id:
        raise CliError("--id is required (or set TEAMFLOW_HANDOFF_ID)")
    return handoff_id


def _resolve_run_id(args):
    run_id = args.run_id or os.environ.get("TEAMFLOW_RUN_ID")
    if not run_id:
        raise CliError(
            "--run-id is required (or set TEAMFLOW_RUN_ID; `teamflow run` "
...
def _add_common(parser):
    ...
    parser.add_argument(
        "--runs-dir",
        default=os.environ.get("TEAMFLOW_RUNS_DIR", DEFAULT_RUNS_DIR),
    )


def build_parser():
    parser = argparse.ArgumentParser(
        description="Record and inspect teamflow handoff state mechanically"
    )
...
def main(argv):
    ...
    try:
        ...
        print(f"teamflow handoff: error: {error}", file=sys.stderr)
        return 1
