#!/usr/bin/env python3
"""Liveness monitor for one teamflow agent process.
...
* refresh ``liveness/<pid>--<role>--<depth>.json`` while the monitored
  process lives, so ``teamflow agents list`` has a fact source; and
...
def main(argv):
    ...
    parser.add_argument(
        "--runs-dir", default=os.environ.get("TEAMFLOW_RUNS_DIR", handoff_state.DEFAULT_RUNS_DIR)
    )
