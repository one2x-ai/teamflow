...
def main(argv):
    parser = argparse.ArgumentParser(
        description="Block until new teamflow events arrive, then return them"
    )
    parser.add_argument("--run-id")
    parser.add_argument(
        "--runs-dir", default=os.environ.get("TEAMFLOW_RUNS_DIR", DEFAULT_RUNS_DIR)
    )
    ...
    run_id = args.run_id or os.environ.get("TEAMFLOW_RUN_ID") or None
    kinds = set()
    ...
    backend = os.environ.get("TEAMFLOW_WAIT_BACKEND", "auto")
